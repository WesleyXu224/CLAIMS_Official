...
def main():
    ...
    # 5. SMPL 数据目录
    parser.add_argument('--smpl_data_dir', default='data/smpl',
                        help='SMPL 资产目录（默认 data/smpl）')
...
# 原代码：
        ...
        for file_path in npy_files:  # 按文件顺序遍历
            ...
            # Generate a local MJCF from the current SMPL body so the conversion
            # runs inside the PHC project instead of depending on an external path.
            smpl_local_robot.load_from_skeleton(
                betas=torch.from_numpy(beta[None,]),
                gender=gender_number,
                objs_info=None,
            )
            os.makedirs("phc/data/assets/mjcf", exist_ok=True)
