...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        # self.opt_cache = {}
        # self.sample_idx = sample_idx#0
        # self.total_num_samples = self.motions['num_samples']#1
        # self.rep_idx = rep_idx#0
        # self.absl_idx = self.rep_idx*self.total_num_samples + self.sample_idx#0
        # self.num_frames = self.motions['motion'][self.absl_idx].shape[-1]#60
        self.num_frames = 180
        self.j2s = joints2smpl(num_frames=self.num_frames, device_id=device, cuda=cuda)


        print(f'Running SMPLify For {npy_path}, it may take a few minutes.')#HERE
        motion_tensor, opt_dict = self.j2s.joint2smpl(self.motions)  # [nframes, njoints, 3]
        self.motions['motion'] = motion_tensor.cpu().numpy()

        # self.bs, self.njoints, self.nfeats, self.nframes = self.motions['motion'].shape
        self.real_num_frames = 180#60
